...
module.exports = function(type, options) {
    return through(function(file) {
        ...
        if (type === 'amd') { contents = makeAMD(contents, file.path, opts); } else if (type === 'commonjs' || type === 'node') { contents = makeCommonJS(contents, file.path, opts); } else if (type === 'hybrid') { contents = makeHybrid(contents, file.path, opts); } else if (type === 'plain') { contents = makePlain(contents, file.path, opts); } else if (type === 'es6') { contents = makeEs6(contents, file.path, opts); } else {
